...
export default function NodePanel({ node, onDelete }: NodePanelProps) {
  ...
  const setNodeContext = useFlowStore((s) => s.setNodeContext);
  const executeNode = useFlowStore((s) => s.executeNode);
  const setCurrentNodeId = useFlowStore((s) => s.setCurrentNodeId);
  ...
  async function handleExecuteNode() {
    const { data, error } = await executeNode(node.id);
    if (error) {
