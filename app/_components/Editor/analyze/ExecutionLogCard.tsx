...
export default function ExecutionLogCard({
  ...
  const parsedOutput = execution.output ? JSON.parse(execution.output) : null;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">
                {execution.node_label || "Unknown Node"}
              </h3>
              <p className="text-sm text-gray-500">
                Node ID: {execution.node_id || "N/A"}
              </p>
            </div>
          </div>
      ...
      {isExpand && true && (
        <div className="border-t border-gray-200 p-4 bg-gray-50">
          ...
          <div className="bg-white rounded border p-3 text-sm text-gray-800 font-mono">
            {typeof parsedOutput !== "string" && parsedOutput ? (
              <Link
                href={parsedOutput.reportUrl}
                download="report"
                target="_blank"
              ...
              execution.output.split(" ").slice(0, 10).join(" ")
            ) : (
